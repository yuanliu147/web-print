...
export function getDirection(e: ClientPosition, activeElement: ActiveElement): Direction {
	...
	const isLeft =
		Math.abs(clientX - left) <= RESIZE_DISTANCE ? LEFT_DIRECTION : NONE_DIRECTORY
	const isRight =
		Math.abs(clientX - right) <= RESIZE_DISTANCE ? RIGHT_DIRECTION : NONE_DIRECTORY
	const isBottom =
		Math.abs(clientY - bottom) <= RESIZE_DISTANCE ? BOTTOM_DIRECTION : NONE_DIRECTORY
